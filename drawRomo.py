...
def draw_field(figure, axis, landing_zones, romo_info, field_width, field_height, objects, num):
    ...
    for x in landing_zones:
        rect = matplotlib.patches.Rectangle((x[0], x[1]), x[2], x[3], color=x[4])
        circ = matplotlib.patches.Circle((x[0] + x[2]/2.0, x[1]+x[3]/2), radius = min(x[2]/4.0, x[3]/4.0), color='black')
        axis.add_patch(rect)
        axis.add_patch(circ)
    for o in objects:
        rect = matplotlib.patches.Rectangle((o[0], o[1]), o[2], o[3], color=o[4])
        axis.add_patch(rect)
    romo_rect = matplotlib.patches.Rectangle((romo_info[0], romo_info[1]), romo_info[2], romo_info[3], color=romo_info[4])
    romo_stripe = matplotlib.patches.Rectangle((romo_info[0], romo_info[1]+2*romo_info[3]/5.0), romo_info[2], romo_info[3]/5.0, color='black')
    romo_stripe2 = matplotlib.patches.Rectangle((romo_info[0]+2*romo_info[2]/5.0, romo_info[1]), romo_info[2]/5.0, romo_info[3], color='black')
    ...
    trans = matplotlib.transforms.Affine2D().rotate_deg_around(romo_info[0], romo_info[1], romo_info[5]) + axis.transData
    romo_rect.set_transform(trans)
    romo_stripe.set_transform(trans)
    romo_stripe2.set_transform(trans)
